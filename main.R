...
load_packages <- function() {
  packages <- c("caret", "dplyr", "GA", "doParallel", 
                "extraDistr", "data.table")
...
generate_datasets <- function() {
  ...
  for (i in 1:4) {
    ...
    corr <- i > 2
    if (i %% 2 == 0) {
      df <- dataGen10(p = 30, n = 300, Corr = corr)
    } else {
      df <- dataGen5(p = 15, n = 300, Corr = corr)
    }
    filename <- ifelse(corr, paste0("Dataset_", i, "_Corr.csv"), paste0("Dataset_", i, ".csv"))
...
# Define Ranking and Selection based GA
train_RSGA<- function(data_corr, Exp_Name, dt_name, k){
  require('caret')
  require('GA')
  ## Setting the parameters
  B1=10 # number of outer reps
  R0=10 # number of middle reps
  delta=2 # step size for change in sample size
  maxiter_param <- 100 # maximum number of iterations for GA
                  ...
                  seed = NULL) 
  {
    ...
    for(iter in seq_len(maxiter))
    {
      ...
      #print(Fitness[,1:10])
      # update object
      # saving mean fitness values and variances
      object@population <- Pop
  ...
  #### Refer to the paper for full description and details ####
  ...
  ga_postfit <- function(object, ...){
    ...
    if(object@iter>1){
      ...
      est_err = sqrt(abs(mean(object@fitvar[,best_id], na.rm = T))/mean(object@sampSize[,best_id], na.rm = T))
      #print(object@fitvar[,best_id])
      print(paste0("The opt gap is: ", round(opt_gap,2), " the est err is: ", round(est_err,2)))
      while((max(colSums(object@sampSize))<200) & (est_err>(abs(best_sol)*opt_gap/5))){
        print(paste0("The opt gap is: ", round(opt_gap,2), " the est err is: ", round(est_err,2)))
        sorted_fit= (object@popSize+1) -rank(object@fitness, ties.method = "min")
        best_id= sorted_fit[1]
  ...
  ## Training function for a regression model
  train_model = function(tr_dat, model = "linear"){
    if(model=='linear'){
      return(lm(y~., tr_dat))
    }
    if(model=='quad'){
      return(lm(y~.^2, tr_dat))
    }
  }
  ## Out of sample evaluation of the regression model
  eval_mod = function(mod, tst_dat, MSE = TRUE){
    y_pred = predict(mod, tst_dat)
    if(MSE){
      return(mean((y_pred-tst_dat$y)^2))
    }else{
      return(mean(abs(y_pred-tst_dat$y)))
    }
  }
  func = function(dat, R){
    res = array(NA, dim = R)
    for(r in c(1:R)){
      # set.seed(r)
      tst1 = sample(c(1:nrow(dat)), floor(0.5*nrow(dat)), replace = TRUE)
      tr1 = c(1:nrow(dat))[-tst1]
      mod = train_model(dat[tr1,])
      res[r] = eval_mod(mod, dat[tst1,])
    }
    
    return(mean(res))
  }
  
## Fixed sample size debiased fitness function
  fitness_Debiased_fixed <- function(string) {
    ...
    dat= data_corr[,cc]
    thetahat = func(dat,R)
    theta_B = array(NA, dim= B)
    var_db = array(NA, dim = B)
    theta_BC = array(NA, dim= c(B,C))
    theta_BCD = array(NA, dim= c(B,C,D))
    theta_db = array(NA, dim= B)
    for(i in c(1:B)){
      dat_i = dat[train[,i],] #dat[sample(c(1:nrow(dat)), nrow(dat), replace = TRUE),]
      theta_B[i] = func(dat_i, R)
      for(j in c(1:C)){
        dat_ij = dat_i[sample(c(1:nrow(dat_i)), nrow(dat_i), replace = TRUE),]
        theta_BC[i,j] = func(dat_ij, R)
        for(k in c(1:D)){
          dat_ijk = dat_ij[sample(c(1:nrow(dat_ij)), nrow(dat_ij), replace = TRUE),]
          theta_BCD[i,j,k] = func(dat_ijk, R)
        }
      }
      theta_db[i] = 2*thetahat-2*mean(theta_BC, na.rm = T)+mean(theta_BCD, na.rm = T)
      var_db[i] = var(2*thetahat-2*theta_BC[i,]+theta_BCD[i,,], na.rm = T)
    }
    return(c(-mean(theta_db),-theta_db,var_db))
  }
  
  ## Adaptove Debiased Fitness Function
  fitness_Debiased_adap <- function(string,delta=1,b1_ind) {
    ...
    for(i in c(b1_ind)){
      dat_i = dat[train[,i],]#dat[sample(c(1:nrow(dat)), nrow(dat), replace = TRUE),]
      theta_B[1] = func(dat_i, R)
      for(j in c(1:C)){
        dat_ij = dat_i[sample(c(1:nrow(dat_i)), nrow(dat_i), replace = TRUE),]
        theta_BC[1,j] = func(dat_ij, R)
        for(k in c(1:D)){
          dat_ijk = dat_ij[sample(c(1:nrow(dat_ij)), nrow(dat_ij), replace = TRUE),]
          theta_BCD[1,j,k] = func(dat_ijk, R)
        }
      }
      theta_db[1] = 2*thetahat-2*mean(theta_BC, na.rm = T)+mean(theta_BCD, na.rm = T)
      var_tmp = var(2*thetahat-2*theta_BC[1,]+theta_BCD[1,,],na.rm = T)
    }
    return(c(-mean(theta_db),var_tmp))
    
  }
  
  ### Calling the GA function with defined fitness function
  rsga_model <- RSga("binary", fitness = fitness_Debiased_fixed , nBits = ncol(data_corr)-1,
               names = colnames(data_corr)[1:(ncol(data_corr)-1)],
               selection = ga_selectionRS,  postFitness = ga_postfit,
               maxiter = maxiter_param, optim = T, popSize=floor(ncol(data_corr)-1),
               pcrossover = 0.8, pmutation = 0.35,parallel = T, run=20
  )
  
  selected.cols <- na.omit(colnames(data_corr)[-ncol(data_corr)][which(rsga_model@solution[1,]==1)])
  saveRDS(rsga_model,paste0("/home/kvahdat/Data/ch4/GA_",Exp_Name,"_",dt_name,"_",k,".rds"))
...
# Run RSGA on datasets
run_rsga <- function(datasets, N, seeds) {
  for (dt in datasets) {
    # setwd("/home/kvahdat/Data/ch4")
    full <- fread(paste0("./", dt, ".csv"))
    full <- as.data.frame(full)
    sampleresult <- array(0, dim = c(3, N), dimnames = list(c("User time", "TruePos", "Num"), c(1:N)))
    ...
    for (k in 1:N) {
      print(paste0("Iteration ", k, " Dataset ", dt))
      set.seed(seeds[1, k])
      r <- sample(1:nrow(full), floor(nrow(full) * 0.5))
      
      data_corr <- full[r,]
      results1 <- train_RSGA(data_corr, Exp_Name, dt, k)
      sampleresult[1, k] <- results1[1]
      selectedcols <- results1[-1]
      colresults[1:length(selectedcols), k] <- selectedcols
      
      truevars_ind <- ifelse(substr(dt, nchar(dt) - 4, nchar(dt)) == "Corr", 1:5, 1:10)
      sampleresult[2, k] <- sum(colnames(full)[truevars_ind] %in% selectedcols) / length(truevars_ind)
      sampleresult[3, k] <- length(selectedcols)
    }
    write.csv(colresults, paste0("RSGA_Col_", Exp_Name, "_", dt, ".csv"))
    write.csv(sampleresult, paste0("RSGA_Res_", Exp_Name, "_", dt, ".csv"))
  }
}
...
main <- function() {
  ...
  N <- 5 # number of Macro-replication
  P <- 100 # number of post-processing
  B <- 30 # maximum number of micro reps
  
  clusters <- 20 # change based on your computing cores
  cl <- makeCluster(clusters)
  registerDoParallel(cl)
  
  # The experiment name will be used for saving the model and its outputs
  Exp_Name <- "OCBA_FCIB"
  
  # Create seed matrix for reproducibility
  set.seed(1)
